...
def xlsx_to_utf8_csv_in_memory(xlsx_file):
    try:
        # Excel 파일을 읽을 때 다양한 인코딩을 시도합니다.
        encodings = ['utf-8', 'cp949', 'euc-kr', 'iso-8859-1']
        df = None
        for encoding in encodings:
            try:
                df = pd.read_excel(xlsx_file, engine='openpyxl')
                print(f"파일을 성공적으로 읽었습니다. 인코딩: {encoding}")
                break
            except UnicodeDecodeError:
                continue
        
        if df is None:
            raise ValueError("지원되는 모든 인코딩으로 파일을 읽는 데 실패했습니다.")
        ...
        # CSV 데이터 내용 확인
        print("CSV 데이터 미리보기:")
        print(csv_buffer.read(500))  # 처음 500자만 출력
        csv_buffer.seek(0)
...
def process_data(df):
    ...
    for new_col, old_col in column_mapping.items():
        if old_col in df.columns:
            ...
            print(f"경고: '{old_col}' 열을 찾을 수 없습니다.")
...
def main():
    ...
    # 출력 파일명 생성
    output_file = f"{current_date}_bot_엔도젠_형식화.xlsx"
