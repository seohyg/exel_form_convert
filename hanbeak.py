...
def process_data(df):
        ...
        if isinstance(old_cols, list):
            for old_col in old_cols:
                if old_col in df.columns:
                    ...
                    print(f"'{old_col}' 열을 '{new_col}' 열로 매핑했습니다.")
                    break
            else:
                print(f"경고: '{new_col}' 열에 대한 매핑을 찾을 수 없습니다.")
        elif old_cols in df.columns:
            new_df[new_col] = df[old_cols]
            print(f"'{old_cols}' 열을 '{new_col}' 열로 매핑했습니다.")
        else:
            print(f"경고: '{old_cols}' 열을 찾을 수 없습니다.")
    ...
    new_df['매입가'] = new_df['매입가'].apply(safe_numeric_conversion)
    
    new_df['소비자가'] = np.round(new_df['매입가'] * 1.43, -1)
    new_df['판매가격'] = np.round(new_df['매입가'] * 1.3, -1)
    
    for col in ['매입가', '소비자가', '판매가격']:
        new_df[col] = new_df[col].fillna(0).astype(int).apply(lambda x: f"{x:,}")
    
    new_df = new_df[new_columns]
    
    new_df = new_df[new_df['상품명'].notna() & (new_df['상품명'] != '')]
...
def main():
    ...
    # 출력 파일명 생성
    output_file = f"{current_date}_bot_한백샵_형식화.xlsx"
